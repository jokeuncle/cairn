"""Runtime configuration — environment variables with local-first defaults.

This module lives in ``core`` (not ``cli``) so lower layers such as
:mod:`cairn.providers` can read configuration without importing the CLI layer,
keeping dependencies pointing one way (CLAUDE.md P6). The legacy import path
``cairn.cli.config`` re-exports these names for backward compatibility.

Per CLAUDE.md P4 ("local-first must always work"), the defaults target a local
Ollama instance and require no API key.
"""
